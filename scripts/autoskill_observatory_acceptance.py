...
ACCEPTANCE_CRITERIA: tuple[ObservatoryItem, ...] = (
    ...
    ObservatoryItem("21.23", "Autonomy pages expose evidence fidelity, raw-vault policy state, LLM semantic adjudications, deterministic admissibility checks, threshold policy versions, fallback ladders, threshold-deadlock findings, replay/canary corpus episodes, and administrative escalations.", ("semantic_adjudication, autonomy_orchestrator, replay_corpus, administrative_escalation stations", "playbook detail and issue microscope autonomy payloads", "broker replay corpus UI/API")),
    ObservatoryItem("21.24", "Evidence-fidelity views distinguish full-autonomy support from degraded hash-only, metadata-only, redacted-derivative, declassified-summary, and raw-vault-linked states.", ("data_quality/content_policy envelopes in services/observatory.py", "raw_content_available=false summaries", "replay synthesis skips unsupported evidence-fidelity tiers")),
...
DEVELOPER_CHECKLIST: tuple[ObservatoryItem, ...] = (
    ObservatoryItem("24.auto.1", "Add read models and routes for evidence fidelity, raw-vault audit, semantic adjudication, autonomy decisions, replay synthesis, threshold deadlocks, and administrative escalation.", ("autonomy_adjudication subsystem stations", "broker replay corpus admin routes", "memory quarantine/control-flow read models", "playbook detail autonomy fields")),
    ...
    ObservatoryItem("24.auto.3", "Implement autonomy decision microscope pages with evidence links, hard invariants, soft thresholds, fallback ladder, and audit references.", ("generic issue/object microscope payloads", "trace replay policy/gate badges", "playbook detail supporting records and actions")),
    ObservatoryItem("24.auto.4", "Implement replay-corpus synthesis pages showing LLM-synthesized redacted intent and deterministic validation without raw exposure.", ("Broker Replay Corpus frontend tab", "broker replay episode metadata raw_prompt_stored=false", "/v1/broker/replay-episodes/synthesize")),
    ...
    ObservatoryItem("24.auto.6", "Add visual and API tests for evidence-insufficient, soft-threshold fallback, threshold deadlock, administrative escalation, and raw-reveal denial states.", ("visual-regression-fixtures.json covers degraded/stale/security/rollback states", "test_observatory_api.py playbook and issue-board coverage", "test_observatory_acceptance_report.py acceptance crosswalk")),
