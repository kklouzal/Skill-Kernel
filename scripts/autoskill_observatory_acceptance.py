...
ACCEPTANCE_CRITERIA: tuple[ObservatoryItem, ...] = (
    ObservatoryItem("21.1", "The Observatory container serves the web UI and API from a configurable /admin base path.", ("sidecar/autoskill/observatory_main.py", "containers/observatory/Dockerfile", "sidecar/autoskill/observatory/vite.config.ts")),
    ...
    ObservatoryItem("21.3", "The overview graph shows every SkillKernel pipeline station and reflects live health.", ("30 StationDefinition entries in sidecar/autoskill/services/observatory.py", "test_observatory_api.py asserts 30 stations and 9 subsystems")),
    ObservatoryItem("21.4", "The overview graph uses custom station cards, semantic edges, label chips, subsystem lanes, status halos, selected/hover/focus states, and theme-token alignment.", ("sidecar/autoskill/observatory/src/components/AssemblyLine.tsx", "sidecar/autoskill/observatory/src/styles.css")),
    ...
    ObservatoryItem("21.6", "Visual effects are performance-safe, data-backed, adaptive, and optional; WebGL failure or reduced-motion mode does not remove informational value.", ("sidecar/autoskill/observatory/src/components/ParticleLayer.tsx", "reducedMotion gate in App.tsx")),
    ObservatoryItem("21.7", "Overview visual fixtures cover healthy, degraded, blocked, security, regression, freeze, context-pressure, historical-bootstrap, stale telemetry, reduced-motion, low-power, WebGL fallback, and high-load soak states.", ("sidecar/autoskill/observatory/fixtures/visual-regression-fixtures.json", "scripts/autoskill_observatory_fixtures.py", "regression, freeze, historical_bootstrap, stale telemetry, and high-load fixture scenarios", "reduced-motion and particle fallback frontend paths")),
    ObservatoryItem("21.8", "Each subsystem has an intermediate workcell lens showing cross-component flow, conversion, bottlenecks, data quality, traces, issues, and playbooks.", ("SUBSYSTEMS in sidecar/autoskill/services/observatory.py", "Workcells component in App.tsx")),
    ...
    ObservatoryItem("21.12", "Ordinary refresh cycles update visible components in place without full-route flash, graph blanking, chart blanking, viewport reset, tab reset, selected-object loss, or broad component remount unless justified.", ("stable snapshot signatures in App.tsx", "AssemblyLine structuralKey layout preservation")),
    ...
    ObservatoryItem("21.17", "The UI can replay an individual trace or evolution transaction through the pipeline without re-executing work.", ("TraceAndInspector in App.tsx", "/admin/api/v1/replay/traces/{trace_id}", "trace replay safety assertions in test_observatory_api.py")),
    ObservatoryItem("21.18", "Skill pages show SkillIR, SkillGraphIR, compiled artifacts, support files, context budget, scanner/evaluator state, usage attribution, and rollback links.", ("SkillsAndTopology in App.tsx", "/admin/api/v1/skills and /admin/api/v1/context/artifacts")),
    ObservatoryItem("21.19", "Topology pages show create/improve/compose/decompose lineage, dependency, conflict, shadowing, supersession, and external-skill relationships.", ("/admin/api/v1/topology", "topology stations and operation metrics in services/observatory.py", "governance.evolution_transactions.metrics topology transaction review")),
    ...
    ObservatoryItem("21.23", "Autonomy pages expose evidence fidelity, raw-vault policy state, LLM semantic adjudications, calibration observations, reliability metrics, deterministic admissibility checks, threshold policy versions, fallback ladders, threshold-deadlock findings, replay/canary corpus episodes, and administrative escalations.", ("semantic_adjudication, autonomy_orchestrator, replay_corpus, administrative_escalation stations", "/admin/api/v1/adjudications", "/admin/api/v1/autonomy/decisions", "/admin/api/v1/autonomy/calibration/observations", "/admin/api/v1/autonomy/calibration/metrics", "/admin/api/v1/autonomy/policies", "/admin/api/v1/autonomy/threshold-deadlocks", "/admin/api/v1/escalations", "broker replay corpus UI/API")),
    ObservatoryItem("21.24", "The UI distinguishes degraded evidence mode from full-autonomy mode and never implies that hash-only or metadata-only telemetry can support user-intent reconstruction.", ("/admin/api/v1/evidence/fidelity", "/admin/api/v1/raw-vault/summary", "admin_evidence_fidelity_status", "replay synthesis skips unsupported evidence-fidelity tiers")),
    ...
    ObservatoryItem("21.26", "Model/embedding pages expose qualification, structured-output failure, timeout, retry, and error health without dollar-cost analysis.", ("/admin/api/v1/model-profile and /admin/api/v1/embedding-profile", "llm_invocation object microscope", "model_embedding station", "no cost analytics in acceptance non-goals")),
    ObservatoryItem("21.27", "Operator action pages expose action requests, policy checks, idempotency, confirmation state, linked jobs, and audit records.", ("Admin action gateway in App.tsx", "/admin/api/v1/actions, /admin/api/v1/actions/summary, and /admin/api/v1/actions/audit")),
...
DEVELOPER_CHECKLIST: tuple[ObservatoryItem, ...] = (
    ObservatoryItem("24.auto.1", "Add read models and routes for evidence fidelity, raw-vault audit, semantic adjudication, autonomy decisions, calibration observations, reliability metrics, replay synthesis, threshold deadlocks, and administrative escalation.", ("admin_evidence_fidelity_status", "admin_autonomy_decision_status", "autonomy_calibration_observations", "autonomy_reliability_metrics", "admin_semantic_adjudication_status", "admin_administrative_escalation_status", "broker replay corpus admin routes", "memory quarantine/control-flow read models")),
    ...
    ObservatoryItem("24.1", "Add admin API module to the Observatory container.", ("sidecar/autoskill/api/app.py admin routes", "sidecar/autoskill/observatory_main.py")),
    ...
    ObservatoryItem("24.21", "Build all component cockpits.", ("generic cockpit over all 30 stations",)),
    ...
    ObservatoryItem("24.31", "Add guarded action dialogs.", ("Admin action-dialog confirmation flow in observatory/src/App.tsx", "Admin dry-run action gateway buttons and policy receipts", "action summary and audit route coverage")),
    ...
    ObservatoryItem("24.35", "Add raw-content access safeguards and browser security headers.", ("content policy fields and browser-session CSRF headers",)),
