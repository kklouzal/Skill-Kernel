...
ACCEPTANCE_CRITERIA: tuple[ObservatoryItem, ...] = (
    ...
    ObservatoryItem("21.23", "Storage pages expose migration state, read-model freshness, pgvector/index status, and retention backlog.", ("storage_db station", "/admin/api/v1/storage")),
    ObservatoryItem("21.24", "Model/embedding pages expose qualification and health without dollar-cost analysis.", ("/admin/api/v1/model-profile and /admin/api/v1/embedding-profile", "model_embedding station")),
    ObservatoryItem("21.25", "Operator action pages expose action requests, policy checks, idempotency, confirmation state, linked jobs, and audit records.", ("Admin action gateway in App.tsx", "/admin/api/v1/actions and /admin/api/v1/actions/audit")),
    ObservatoryItem("21.26", "Observatory self-health pages expose admin API health, live-stream gaps, frontend diagnostics, and read-model staleness.", ("observatory_admin station", "/admin/api/v1/observatory and health routes")),
    ObservatoryItem("21.27", "Operator actions are role-checked, confirmation-gated when required, idempotent, policy-controlled, and audited.", ("action_receipt in services/observatory.py", "_record_observatory_action in api/app.py")),
    ObservatoryItem("21.28", "The UI defaults to redacted content and does not expose raw content without explicit authorization.", ("content_policy raw_available=false across admin routes", "raw reveal action gate")),
    ObservatoryItem("21.29", "Admin streaming and dashboard queries do not block core processing.", ("bounded Observatory store/read-model routes", "WebSocket/SSE snapshot fallback tests")),
    ObservatoryItem("21.30", "Reduced-motion and low-power modes preserve informational value.", ("reducedMotion state in App.tsx", "prefers-reduced-motion CSS")),
    ObservatoryItem("21.31", "Component health is based on the required signal contract and never reports healthy when required telemetry is missing.", ("REQUIRED_METRICS_BY_FAMILY in services/observatory.py", "zero-count and missing-signal tests")),
    ObservatoryItem("21.32", "Pipeline invariant failures create issue-board entries and deep links.", ("pipeline invariants in services/observatory.py", "invariant route tests")),
    ObservatoryItem("21.33", "Baseline comparison supports bounded time-window and object-version comparisons without changing autonomous policy.", ("/admin/api/v1/comparisons routes", "comparison persistence tests")),
    ObservatoryItem("21.34", "Diagnostic bundles can be generated with redaction by default and audited access.", ("/admin/api/v1/diagnostics/bundles routes", "diagnostic bundle tests")),
    ObservatoryItem("21.35", "The interface meets configured accessibility, keyboard-navigation, reduced-motion, and low-power requirements.", ("semantic buttons/labels in App.tsx", "reduced-motion controls and CSS media query")),
    ObservatoryItem("21.36", "The visual design delivers assembly-line overview, workcell views, station cockpits, and object microscope behavior.", ("AssemblyLine, Workcells, Cockpit, TraceAndInspector, and Inspector components",)),
